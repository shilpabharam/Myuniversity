(function($) {
	$.fn.psteps = function(options) {
		...
		all_elements
				.each(function() {
					...
					// Check for the psteps class. If it has it, we've already
					// transformed this element.
					if (psteps.hasClass("steps-transformed"))
						return true;
					...
					// since they just have a pointer to the defaults if we
					// don't.
					// psteps.example_property =
					// psteps.example_property.slice();

					// Step submit and next button variables
					var all_steps = psteps.find('.step-content'), all_titles = psteps
							.find('.step-title'), step_names = psteps
							.find('.step-name'), next_button = psteps
							.find('.next-button'), back_button = psteps
							.find('.back-button'), send_button = psteps
							.find('.submit-button'), toggle_buttons = psteps
							.find('.next-button, .submit-button'), num_steps = psteps
							.find('.step-title').length, first_time = true;
					...
					// Function that takes step names from titles and makes a
					// heading in
					// the step content.
					psteps.make_step_content_headings = function() {
						psteps
								...
								.each(
										function(r) {
											var step_name = $(this).find(
													'.step-name'), step_content = psteps
													.find('.step-content')
													.eq(r);
											if (step_name.length == 1) {
												var hr = opts.content_headings_hr ? '<hr/>'
														: '', heading = '<div class="step-content-name"><'
														+ opts.content_headings_element
														+ '>'
														+ step_name.html()
														+ '</'
														+ opts.content_headings_element
														+ '>' + hr + '</div>'
												if (opts.content_headings_after != false
														&& step_content
																.find(opts.content_headings_after).length > 0)
													step_content
															.find(
																	opts.content_headings_after)
															.after(heading)
												else
													step_content
															.prepend(heading);
					...
					if (opts.content_height_equalize
							|| opts.steps_height_equalize
							|| opts.steps_width_percentage
							|| opts.shrink_step_names) {
						// Get original max height for equalizing title heights.
						if (opts.steps_height_equalize) {
							var titles = psteps.find('.step-title'), original_height = psteps
									.get_max_height(titles);
						}
						if (opts.content_height_equalize) {
							// going to use a time out here too, so the height
							// is fully determined.
							var step_content = psteps.find('.step-content');
							step_content.addClass('clearfix');
							var original_content_height = psteps
									.get_max_height(step_content);
						}
						$(window)
								.resize(
										function() {
											// Ensure horizontal step widths
											// always look good
											// (responsive/mobile)
											...
											// When viewport is small, do not
											// display step names. Show numbers
											// only.
											if (opts.shrink_step_names) {
												// Time outs needed so that the
												// following can remove content
												// names.
					...
					// Function for adjusting progress title bars on textarea
					// change.
					...
					psteps.check_progress_titles = function() {
						psteps
								...
								.each(
										function(i) {
											var cur_step = $(this), class_to_add = 'pstep'
													+ (i + 1);
											cur_step.addClass(class_to_add);
											// this title matches the content
											var title = psteps.find(
													'.step-title').eq(i);
											title.addClass(class_to_add);
											// Titles are always colored to
											// indicate progress for
											// present/past steps
											// If you can click titles, colored
											// progress will indicate for future
											// steps too.
											// Custom traversal still searches
											// steps-visited to know what to
											// validate for, and remember this
											// part does not handle traversal,
											// but validation.
											// We want to validate for steps
											// besides the custom ones, we just
											// dont want users to go to steps
											// that havent been saved yet (and
											// refreshed to just a view.).
											var validate_result;
											if ((opts.traverse_titles == 'visited' && title
													.hasClass('step-visited'))
													|| (opts.traverse_titles == 'custom' && title
															.hasClass('step-visited'))
													|| (opts.traverse_titles == 'never' && title
															.hasClass('step-visited'))
													|| opts.traverse_titles == 'always') {
												validate_result = opts.validation_rule
														.call(cur_step);
												if (validate_result == 'warning') {
													title
															.removeClass(
																	'step-error btn-info btn-success btn-warning btn-danger step-info-error step-info-incomplete')
															.addClass(
																	'step-warning step-info-warning');
													psteps
															.trigger_error(cur_step);
												} else if (validate_result == 'error') {
													title
															.removeClass(
																	'step-warning btn-info btn-success btn-warning btn-danger step-info-incomplete step-info-warning')
															.addClass(
																	'step-error step-info-error');
													psteps
															.trigger_error(cur_step);
												} else if (validate_result) {
													title
															.removeClass(
																	'step-warning btn-info btn-warning btn-danger step-error step-info-error step-info-incomplete step-info-warning')
															.addClass(
																	'btn-success')
													if (opts.check_marks)
														title
																...
																.end()
																.prepend(
																		'<i class="icon-ok step-mark"></i> ');
												} else if (!validate_result) {
													title
															.removeClass(
																	'step-warning btn-info btn-success btn-danger btn-warning step-error step-info-error step-info-warning')
															.addClass(
																	'btn-info step-info-incomplete')
															.find('i.step-mark')
															.remove();
												}
											} else {
												...
												if (validate_result == 'warning') {
													title
															.removeClass(
																	'step-info-incomplete step-info-error')
															.addClass(
																	'step-info-warning');
												} else if (validate_result == 'error') {
													title
															.removeClass(
																	'step-info-incomplete step-info-warning')
															.addClass(
																	'step-info-error');
													psteps
															.trigger_error(cur_step);
													...
													title
															.removeClass('step-info-error step-info-incomplete step-info-warning');
												} else if (!validate_result) {
													title
															.removeClass(
																	'step-info-error step-info-warning')
															.addClass(
																	'step-info-incomplete')
												}
											}
										});
					...
					// Function for toggling send/next buttons as btn-success or
					// btn-info.
					psteps.toggle_buttons_function = function() {
						// Get proper button name
						var next_name = next_button.attr('data-btn-name'), send_name = send_button
								.attr('data-btn-name'), back_name = back_button
								.attr('data-btn-name');
						...
						if (psteps.find('.step-content').last().hasClass(
								'step-active')) {
						...
						// Changes color of send/next buttons based upon
						// completion.
						var active_title = psteps
								.find('.step-title.step-active');
						if (active_title.hasClass('btn-success')
								|| active_title.hasClass('step-warning'))
							toggle_buttons.removeClass('btn-info').addClass(
									'btn-success');
						else if (active_title.hasClass('btn-info'))
							toggle_buttons.removeClass('btn-success').addClass(
									'btn-info');
						else if (active_title.hasClass('step-error'))
							toggle_buttons.removeClass('btn-success btn-info')
									.addClass('btn-danger');

						// Check submit button for all steps if necessary
						if (opts.validate_submit_all_steps) {
							if (all_titles.filter('.btn-info').length
									|| (all_titles.filter('.step-error').length && !opts.ignore_errors_on_submit))
						...
						if (opts.back && !back_button.hasClass('btn-manual')) {
							if (psteps.find('.step-title').first().hasClass(
									'step-active'))
								back_button.hide();
							else {
								var previous_title = psteps.find(
										'.step-title.step-active').prevAll(
										'.step-title:first');
					...
					psteps.go_to_step = function(step_num) {
						var last_active_title = psteps
								.find('.step-title.last-active'), last_active_content = psteps
								.find('.step-content.last-active'), active_step = psteps
								.find('.step-content.step-active'), active_title = psteps
								.find('.step-title.step-active');

						if (step_num > num_steps)
							return;

						var show_step = psteps.find('.step-content').eq(
								step_num - 1), show_title = psteps.find(
								'.step-title').eq(step_num - 1);

						// If you're at go to step, and there's step-resumes,
						// the step-transition-processing
						// was not removed. Remove step-resume class if you
						// don't want to remove the step
						// transition processing class.
						if (psteps.find('.step-resume').length) {
							psteps.find('.step-transition-processing')
									.removeClass('step-transition-processing');
						}
						if (active_title.hasClass('step-transition')
								&& !psteps.find('.step-resume').length
								&& !first_time)
							opts.steps_transition.call(active_step);
						if (active_title.hasClass('step-transition-processing')) {
							psteps.find('.step-resume').removeClass(
									'step-resume');
						...
						active_step.hide().removeClass('step-active').addClass(
								'last-active');
						show_step.show().addClass('step-active step-visited');

						active_title.removeClass('step-active').addClass(
								'disabled last-active');
						show_title.addClass('step-active step-visited')
								.removeClass('disabled');

						// Every step before this one must get step-visited now.
						// This is because if I had step traversal set to
						// visited
						// and I started on step3, I could never see step1 and
						// step2, which if that was the desired affect, those
						// steps should
						// instead be given a no-traverse class. I should be
						// able to to "go back" to steps prior to the one I am
						// on.
						show_step.prevAll('.step-content').addClass(
								'step-visited');
						show_title.prevAll('.step-title').addClass(
								'step-visited');
						...
						if (opts.traverse_titles == 'visited') {
							show_title.prevAll('.step-title').andSelf().css(
									'cursor', 'pointer');
						}
					...
					psteps.go_to_next_step = function() {
						var preceeding_titles = psteps.find(
								'.step-title.step-active').prevAll(
								'.step-title');
						if (opts.skip_no_traverse_on_next) {
							var next_step = psteps.find('.step-title').eq(
									preceeding_titles.length + 1);
							if (next_step.hasClass('step-notraverse')) {
								var new_next_step = next_step.nextAll(
										'.step-title:not(.step-notraverse)')
										.first().prevAll('.step-title').length + 1;
					...
					psteps.go_to_prev_step = function() {
						var preceeding_titles = psteps.find(
								...
								'.step-title');
						if (opts.skip_no_traverse_on_next) {
							var prev_step = psteps.find('.step-title').eq(
									preceeding_titles.length - 1);
							if (prev_step.hasClass('step-notraverse')) {
								var new_prev_step = prev_step.prevAll(
										'.step-title:not(.step-notraverse)')
										.first().prevAll('.step-title').length + 1;
					...
					// Function to set or change the way titles are traversed.
					// Used by
					// the binding events that can be triggered to change the
					// type.
					psteps.change_traverse = function(type) {
						...
						psteps
								.off('click',
										'.step-title.step-active:not(.step-notraverse)');
						psteps
								.off('click',
										'.step-title.step-visited:not(.step-notraverse)');
						if (type == 'always') {
							...
							psteps
									.on(
											...
											'.step-title:not(.step-notraverse)',
											function() {
												var clicked_title = $(this), all_prev = clicked_title
														.prevAll('.step-title');
												psteps
														.go_to_step(all_prev.length + 1);
											});
						} else if (type == 'custom') {
							var selector1 = "." + custom;
							var selector2 = ".step-title." + custom
									+ ":not(.step-notraverse)";
							step_titles.css('cursor', 'default').filter(
									selector1).css('cursor', 'pointer');
							psteps.on('click', selector2, function() {
								var clicked_title = $(this);
								// if the title is the "next" title from the
								// current view,
								// trigger next.
								// if the title is the "previous" title from the
								// current view,
								// trigger previous.
								if (clicked_title.prevAll('.step-title:first')
										.hasClass('step-active'))
									psteps.go_to_next_step();
								else if (clicked_title.nextAll(
										'.step-title:first').hasClass(
										'step-active'))
									psteps.go_to_prev_step();
								else {
									var all_prev = clicked_title
											.prevAll('.step-title');
							...
							step_titles.css('cursor', 'default').filter(
									'.step-visited').css('cursor', 'pointer');
							psteps
									.on(
											'click',
											'.step-title.step-visited:not(.step-notraverse)',
											function() {
												var clicked_title = $(this);
												// if the title is the "next"
												// title from the current view,
												// trigger next.
												// if the title is the
												// "previous" title from the
												// current view,
												// trigger previous.
												if (clicked_title
														.prevAll(
							...
							// this is for never, which this is actually super
							// useful.
							// It allows the user to click on the current step,
							// which
							// may trigger the alert for why a step is showing a
							// warning
							// or an error. Because the "next" button on errors
							// will
							// trigger a click on this step (which shows the
							// error message)
							// from the validation rule. This allows for unique
							// error
						...
						step_titles.filter('.step-notraverse').css('cursor',
								'default');
					};

					// Trigger Error in Title
					psteps.trigger_error = function(step) {
						var step_num = step.prevAll('.step-content').length + 1, title = psteps
								.find('.step-title').eq(step_num - 1);
						if (title.hasClass('step-warning')) {
							title.removeClass('btn-info btn-success').addClass(
									'btn-warning')
							if (opts.check_marks)
								title
										.find('i.step-mark')
										...
										.end()
										.prepend(
												'<i class="icon-remove step-mark"></i> ');
					...
					all_steps.hide().first().addClass(
							'step-visited step-active').show();
					all_titles.addClass('disabled btn').first().addClass(
							'step-visited step-active').removeClass('disabled');
					if (!opts.step_names)
						step_names.hide();
					// Load functions
					...
					if (opts.start_incomplete_step) {
						var incomplete = psteps.find(
								'.step-title.step-info-incomplete').first();
						if (incomplete.length)
							psteps
									.go_to_step(incomplete
											.prevAll('.step-title').length + 1);
						else {
							last = psteps.find('.step-title.btn-success')
									.last();// it will go to the last completed
											// if nothing is incomplete
							psteps
									.go_to_step(last.prevAll('.step-title').length + 1);
						}
					} else if (opts.start_warning_step) {
						var warning = psteps.find(
								'.step-title.step-info-warning').first();
						if (warning.length)
							psteps
									.go_to_step(warning.prevAll('.step-title').length + 1);
						else {
							last = psteps.find('.step-title.btn-success')
									.last();// it will go to the last completed
											// if nothing is incomplete
						...
						var error = psteps.find('.step-title.step-info-error')
								.first();
						if (error.length)
							psteps
									.go_to_step(error.prevAll('.step-title').length + 1);
						else {
							last = psteps.find('.step-title.btn-success')
									.last();// it will go to the last completed
											// if nothing is incomplete
					...
					back_button.click(function() {
						if (opts.use_custom_back_button) {
							var previous_step_num = psteps.find(
									'.step-title.step-active').prevAll(
									'.step-title').length;
							opts.custom_back_button_click.call(undefined,
									previous_step_num);
						} else {
							psteps.go_to_prev_step();
						}
					...
					// An event that triggers a change on how titles will be
					// traversed.
					...
					// Loads the call back for what happens after steps,
					// probably
					...
					// Have yet to find a use for this, but maybe it will be to
					// someone.
					psteps.on('step_error.psteps', '.step-content', function() {
						psteps.trigger_error($(this));
					});

					// When triggered on a step, it will go there, despite
					// traversal settings.
					psteps.on('go_to_step.psteps', '.step-title', function() {
						var cur_step = $(this), preceeding_titles = cur_step
								.prevAll('.step-title');
						psteps.go_to_step(preceeding_titles.length + 1);
					});

					// When triggered on psteps object, it goes to the first
					// incomplete step. Or the last completed if none are
					// incomplete.
					psteps
							.bind(
									'go_to_first_incomplete.psteps',
									function() {
										var incomplete = psteps
												.find(
														'.step-title.step-info-incomplete')
												.first();
										if (incomplete.length)
											psteps
													.go_to_step(incomplete
					...
					// When triggered on psteps object, it goes to the first
					// step with an warning. Or the last completed if none are
					// warnings.
					psteps
							.bind(
									'go_to_first_warning.psteps',
									function() {
										var warning = psteps
												.find(
														'.step-title.step-info-warning')
												.first();
										if (warning.length)
											psteps
													.go_to_step(warning
					...
					// When triggered on psteps object, it goes to the first
					// step with an error. Or the last completed if none are
					// errors.
					psteps
							.bind(
									'go_to_first_error.psteps',
									function() {
										var error = psteps.find(
												'.step-title.step-info-error')
												.first();
										if (error.length)
											psteps
													.go_to_step(error
					...
					// Submit or Next. Checks for success in order to progress.
					// Stops submit if fails.
					toggle_buttons
							.click(function(e) {
								...
								if (this_button.hasClass(opts.btn_wait_class)) {
									setTimeout(
											function() {
												...
												// validation ran and titles are
												// correct run check titles.
												psteps
														.trigger('validate.psteps');
												var active_title = psteps
														.find('.step-title.step-active');
												if (active_title
														.hasClass('step-error')
														&& opts.validate_errors
														&& !opts.validate_next_step) {
													...
													psteps.go_to_next_step();
														...
														&& (opts.validate_submit_all_steps || opts.validate_next_step)) {
													if (opts.use_before_submit)
														alert(opts.before_submit);
													e.preventDefault();
												} else if (opts.validate_next_step
														&& opts.use_before_next)
													alert(opts.before_next);
												else
													psteps.go_to_next_step();
											}, opts.btn_wait_time);
								} else {
									// Just to make sure that the validation ran
									// and titles are correct run check titles.
									psteps.trigger('validate.psteps');
									var active_title = psteps
											.find('.step-title.step-active');
	...
	$.fn.psteps.defaults = {
		...
		// If set to 'custom', only steps with your custom_traverse_class will
		// be traversable.
		traverse_titles : 'never',
		// Custom class you can put on your step titles to use to limit
		// traversal. Set traverse_titles to 'custom'.
		custom_traverse_class : 'custom-traverse',
		// Always have access the first incomplete step always, regardless of
		// step traversal settings.// TO DO
		// traverse_first_incomplete: true, TO DO
		// If step title has step-notraverse, do not allow the previous step to
		// get to it, instead have it go to the next available.
		skip_no_traverse_on_next : true,
		// If step names should be copied to step content and used as headings
		content_headings : false,
		// The element inside the content headings div to wrap around the step
		// name. ie h4
		...
		// False if not used.
		// If a selector is specified but doesn't exist for every step or at all
		// in the DOM tree, then
		// it will display at the beginning of a step by default.
		content_headings_after : false,
		// Step order is part of the step titles, defined by a class of
		// "step-order".
		// When step order is true, step-order shows all the time. If
		// shrink_step_names
		// is true, step order will show at specified viewport sizes even when
		// step
		// order is false.
		step_order : true,
		// Step names will be hidden always if this statement is set to false,
		// but you can still specify step names so that content headings can
		// still exist.
		step_names : true,
